...
resource "aws_s3_bucket" "tf-s3" {
  bucket = "latif-tf-test-bucket-addwhateveryouwant"
}
```
...
- The `terraform apply -refresh-only` command is used to update the state file with the real-world infrastructure. This can be used to detect any drift from the last-known state, and to update the state file. First, check the current state of your resources with `terraform state list`. Then go to the AWS console and delete your S3 bucket `latif-tf-test-bucket-addwhateveryouwant`. Display the state list again and refresh the state. Run the following commands.
...
resource "aws_s3_bucket" "tf-s3" {
  bucket = "latif-tf-bucket-addwhateveryouwant-new"
  #bucket = "latif-tf-bucket-addwhateveryouwant"
}
```
...
variable "s3_bucket_name" {
  default = "latif-s3-bucket-variable-addwhateveryouwant"
}
...
```go
variable "s3_bucket_name" {
#   default     = "latif-new-s3-bucket-addwhateveryouwant"
}
```
...
```bash
terraform plan -var="s3_bucket_name=latif-new-s3-bucket-2"
```
...
```bash
export TF_VAR_s3_bucket_name=latif-env-varible-bucket
terraform plan
```
...
- Create a file name `latif.tfvars`. Add the followings.

```go
s3_bucket_name = "latif-tfvar-bucket"
```

- Run the command below.

```bash
terraform plan --var-file="latif.tfvars"
```

- Create a file named `latif.auto.tfvars`. Add the followings.

```go
s3_bucket_name = "latif-auto-tfvar-bucket"
```
